...
shinyServer(function(input, output, session) {
  ...
  output$isUnadjusted <- reactive({
    row <- selectedRow()
    isUnadjusted <- !is.null(row) && row$analysisId %in% c(1,2)
    if (isUnadjusted) {
  ...
  outputOptions(output, "isUnadjusted", suspendWhenHidden = FALSE)
  ...
  output$mainTable <- renderDataTable({
    ...
    table$analysisOrder <- match(table$analysisId, c(1, 3, 5, 2, 4, 6))
    table <- table[order(table$analysisOrder), ]
  ...
  output$table1Table <- renderDataTable({
    ...
    if (is.null(row)) {
      ...
      if (row$analysisId %in% c(3,4)) {
        spec <- "Table1SpecsSelect.csv"
      } else {
        spec <- "Table1Specs.csv"
      }
  ...
  psDistPlot <- reactive({
    row <- selectedRow()
    if (is.null(row) || row$analysisId %in% c(1,2)) {
      return(NULL)
    } else {
      targetId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$target]
      comparatorId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$comparator]
  ...
  balancePlot <- reactive({
    ...
    if (is.null(bal) || nrow(bal) == 0) {
      ...
      row <- selectedRow()
      if (row$analysisId %in% c(1,2)) {
        return(NULL)
      } else if (row$analysisId %in% c(3,4)) {
        writeLines("Plotting covariate balance")
        plot <- plotCovariateBalanceScatterPlot2(balance = bal,
