  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    temporalCharacterizationOutput <-
      shiny::reactive(x = {
        ...
        previouslyExtractedData <- characterizationOutput()
        
        usePreviouslyExtractedData <- FALSE
        if (all(
          hasData(previouslyExtractedData$covariateValue),
          hasData(selectedDatabaseId()),
          selectedDatabaseId() %in% c(selectedDatabaseIds())
        )) {
          if (nrow(
            previouslyExtractedData$covariateValue %>%
            dplyr::filter(.data$databaseId %in% c(selectedDatabaseId()))
          ) > 0) {
            usePreviouslyExtractedData <- TRUE
          }
        }

        data <- NULL
        if (usePreviouslyExtractedData) {
