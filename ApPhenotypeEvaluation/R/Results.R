...
getValidationMetricsResult <- function(dataSource = .GlobalEnv,
                                       cohortIds,
                                       databaseIds) {
  if (is(dataSource, "environment")) {
    data <- validationMetrics %>% 
      dplyr::filter(.data$databaseId %in% !!databaseIds & .data$cohortId %in% cohortIds) %>%
      dplyr::select(-cohortId)
  } else {
    sql <- "SELECT *
            FROM  @results_database_schema.validation_metrics
            WHERE database_id in (@database_id)
            {@cohort_ids != ''} ? {  AND cohort_id in (@cohort_ids)}
            ;"
    data <- renderTranslateQuerySql(connection = dataSource$connection,
...
getDiagnosticModelResult <- function(dataSource = .GlobalEnv,
                                     cohortIds = NULL,
                                     databaseId) {
  if (is(dataSource, "environment")) {
    data <- diagnosticModel %>% 
      dplyr::filter(.data$databaseId %in% !!databaseId) %>%
      dplyr::relocate(databaseId, Beta, Covariate, ConceptId)
    data$Beta <- round(data$Beta, 3)
  } else {
    sql <- "SELECT *
            FROM  @results_database_schema.diagnostic_model
            WHERE database_id in (@database_id)
            {@cohort_ids != ''} ? {  AND cohort_id in (@cohort_ids)}
            ;"
    data <- renderTranslateQuerySql(connection = dataSource$connection,
