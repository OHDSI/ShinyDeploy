shiny::shinyServer(function(input,
                            output,
                            session) {
  ...
  cohortSubset <- shiny::reactive({
    return(cohort %>% dplyr::arrange(.data$cohortId))
  })
  ...
  cohortDefinitionCirceRDetails <- shiny::reactive(x = {
    ...
    if (nrow(selectedCohortDefinitionRow()) > 0) {
      details <- list()
      circeExpression <- CirceR::cohortExpressionFromJson(expressionJson = data$json)
      circeExpressionMarkdown <- CirceR::cohortPrintFriendly(circeExpression)
      circeConceptSetListmarkdown <- CirceR::conceptSetListPrintFriendly(circeExpression$conceptSets)
      details <- data
      details$circeConceptSetListmarkdown <- circeConceptSetListmarkdown
      details$htmlExpressionCohort <- convertMdToHtml(circeExpressionMarkdown)
      details$htmlExpressionConceptSetExpression <- convertMdToHtml(circeConceptSetListmarkdown)
  ...
  getConceptSetDataFrameFromConceptSetExpression <- function(conceptSetExpression) {
    ...
    if ('CONCEPT_ID' %in% colnames(conceptSetExpressionDetails)) {
      ...
      colnames(conceptSetExpressionDetails) <- snakeCaseToCamelCase(colnames(conceptSetExpressionDetails))
    }
    return(conceptSetExpressionDetails)
  }

  getConceptSetDetailsFromCohortDefinition <- function(cohortDefinitionExpression) {
    ...
    conceptSetExpression <- expression$ConceptSets %>%
      dplyr::bind_rows() %>%
      dplyr::mutate(json = RJSONIO::toJSON(x = .data$expression, pretty = TRUE))

    conceptSetExpressionDetails <- list()
    i <- 0
    for (id in conceptSetExpression$id) {
      i <- i + 1
      conceptSetExpressionDetails[[i]] <- getConceptSetDataFrameFromConceptSetExpression(conceptSetExpression =  conceptSetExpression[i, ]$expression$items) %>%
        dplyr::mutate(id = conceptSetExpression[i,]$id) %>%
        dplyr::relocate(.data$id) %>%
        dplyr::arrange(.data$id)
    }
    conceptSetExpressionDetails <- dplyr::bind_rows(conceptSetExpressionDetails)
  ...
  cohortDefinistionConceptSetExpression <- shiny::reactive({
    ...
    expression <- getConceptSetDetailsFromCohortDefinition(cohortDefinitionExpression = expression)
    return(expression)
  })
  ...
  cohortDefinitionConceptSets <- shiny::reactive(x = {
    ...
    data <- cohortDefinistionConceptSetExpression()$conceptSetExpressionDetails
    data <- data %>%
  ...
  getResolvedOrMappedConceptSetForAllDatabase <- shiny::reactive(x = {
    ...
    output <- resolveMappedConceptSet(
  ...
  getConceptCountForAllDatabase <- shiny::reactive(x = {
    row <- selectedCohortDefinitionRow()
    if (is.null(row) ||
        is.null(cohortDefinitionConceptSetExpressionRow()$id)) {
      return(NULL)
  ...
  getResolvedOrMappedConcepts <- shiny::reactive({
    ...
    if (length(databaseIdToFilter) > 0) {
      ...
      if (!is.null(resolvedOrMappedConceptSetForAllDatabase) &&
        ...
        source <- (input$conceptSetsType == "Mapped")
        if (source) {
          ...
          if (!is.null(data) && nrow(data) > 0) {
            ...
            # data$resolvedConcept <- as.factor(data$resolvedConcept)
          } else {
            data <- NULL
          }
        } else {
  ...
  output$cohortDefinitionIncludedResolvedConceptsTable <- DT::renderDataTable(expr = {
  ...
  output$cohortDefinitionMappedConceptsTable <- DT::renderDataTable(expr = {
    data <- getResolvedOrMappedConcepts()
  ...
  shiny::observeEvent(input$validationMetricsInfo, {
    showInfoBox("Validation metrics",
                "html/validationMetricsInfo.html")
  })

  shiny::observeEvent(input$diagnosticModelInfo, {
    showInfoBox("Diagnostic Model",
                "html/diagnosticModelInfo.html")
  })
  ...
  getValidationMetricsResultReactive <- shiny::reactive(x = {
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(length(cohortIds()) > 0, "No cohorts chosen"))
    data <- getValidationMetricsResult(
      dataSource = dataSource,
      databaseIds = databaseIds(),
      cohortIds = cohortIds()
    )
  ...
  output$validationMetricsTable <- DT::renderDataTable(expr = {
    data <- getValidationMetricsResultReactive()
  ...
  getDiagnosticModelResultReactive <- shiny::reactive(x = {
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    #validate(need(length(cohortIds()) > 0, "No cohorts chosen"))
    data <- getDiagnosticModelResult(
      dataSource = dataSource,
  ...
  output$diagnosticModelTable <- DT::renderDataTable(expr = {
    data <- getDiagnosticModelResultReactive()
    table <- DT::datatable(data,
                           rownames = FALSE,
                           class = "stripe compact")
    return(table)
