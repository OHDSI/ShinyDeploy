...
if (!exists("shinySettings")) {
  if (Sys.getenv("shinydbServer") != "" && Sys.getenv("heradbSchema") != "") {
    ...
    shinySettings <- list(storage = "s3", dataFolder = Sys.getenv("OHDSI_SHINY_DATA_BUCKET"), dataFile = "HERA/apcb19uv_PreMerged.RData")
  } else {
...
# This must match the featureTimeWindow.csv from the HERA study
timeWindow <- data.frame(windowId=c(1:3), name=c("-365d to -1d", "index", "1d to 365d"))
