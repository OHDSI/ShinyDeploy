...
shinyServer(function(input, output, session) {
  ...
  resultSubset <- reactive({
    ...
    results$dbOrder <- match(results$databaseId, c("CCAE", "MDCD", "MDCR", "Optum", "Meta-analysis"))
    results <- results[order(results$analysisId, results$dbOrder), ]
    results$dbOrder <- NULL
    if (blind) {
  ...
  output$isMetaAnalysis <- reactive({
    ...
    if (isMetaAnalysis) {
      ...
      hideTab("detailsTabsetPanel", "Target-analytic balance")
      hideTab("detailsTabsetPanel", "Target-analytic plot")
      ...
      showTab("detailsTabsetPanel", "Target-analytic balance")
      showTab("detailsTabsetPanel", "Target-analytic plot")
    ...
    if (blind & isMetaAnalysis) {
      hideTab(inputId = "detailsTabsetPanel", target = "Forest plot")
    }
  ...
  taBal <- reactive({
    ...
    if (is.null(row)) {
      ...
      targetId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$target]
      comparatorId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$comparator]
      taBal <- getTaBalance(connection = connection,
                            targetId = targetId,
                            comparatorId = comparatorId,
                            analysisId = row$analysisId,
                            databaseId = row$databaseId)
  ...
  output$targetAnalyticBalanceCaption <- renderUI({
    row <- selectedRow()
    if (is.null(row)) {
      return(NULL)
    } else {
      text <- "<strong>Table 2.</strong> Target vs analytic cohort characteristics"
      return(HTML(sprintf(text)))
    }
  })

  output$targetAnalyticTable <- renderDataTable({
    row <- selectedRow()
    if (is.null(row)) {
      return(NULL)
    } else {
      targetAnalyticBalance <- taBal()
      if (nrow(targetAnalyticBalance) == 0) {
        return(NULL)
      }

      taBal <- prepareTaBalanceTable(targetAnalyticBalance)
      options <- list(searching = TRUE,
                      ordering = TRUE,
                      paging = TRUE,
                      bInfo = TRUE)
      table <- datatable(taBal,
                         options = options,
                         rownames = FALSE,
                         escape = FALSE,
  ...
  # target analytic plot =======================================================

  output$targetAnalyticPlot <- renderPlot({
    row <- selectedRow()
    if (is.null(row)) {
      return(NULL)
    } else {
      ...
      plot <- plotCovariateBalanceScatterPlot2(targetAnalyticBalance)
    }
    return(plot)
  })
  ...
  output$table1Caption <- renderUI({
    ...
    if (is.null(row)) {
      ...
      text <- "<strong>Table 2.</strong> Characteristics before and after propensity score adjustment, showing the (weighted)
  ...
  psDistPlot <- reactive({
    ...
    if (is.null(row)) {
      ...
      attrition <- getAttrition(connection = connection,
                                ...
                                analysisId = row$analysisId,
                                databaseId = row$databaseId)
      targetSize <- attrition$targetPersons[attrition$description == "Have at least 1 days at risk"] # need to exposure export equipoise values rather than calculate on fly
      comparatorSize <- attrition$comparatorPersons[attrition$description == "Have at least 1 days at risk"]
      plot <- plotPs(ps, input$target, input$comparator, targetSize, comparatorSize)
      return(plot)
    }
  })
  ...
  kaplanMeierPlot <- reactive({
    row <- selectedRow()
    if (blind || is.null(row) || is.na(row$rr)) {
      return(NULL)
    } else {
      targetId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$target]
      comparatorId <- exposureOfInterest$exposureId[exposureOfInterest$exposureName == input$comparator]
