...
plotPs <- function(ps,
  ...
  if (is.null(ps)) {
    ...
    labelsRight <- sprintf("Empirical equipoise: %2.1f%%", totalFraction * 100)
...
plotCovariateBalanceScatterPlot <- function(balance,
  ...
  # labelsRight <- sprintf("Covariates ASMD > 0.1: %2.5f%%", nImbalancedCovars / nCovars * 100)
  ...
  plot <- plot + ggplot2::geom_label(x = limits[2],
                                     y = limits[2],
                                     hjust = "right",
                                     vjust = "top",
                                     alpha = 1,
...
plotScatter <- function(controlResults) {
  ...
  uncalibratedNull <- EmpiricalCalibration::fitNull(d$logRr[d$yGroup == "Uncalibrated"], d$seLogRr[d$yGroup == "Uncalibrated"])
  ...
  calibratedNull <- EmpiricalCalibration::fitNull(d$logRr[d$yGroup == "Calibrated"], d$seLogRr[d$yGroup == "Calibrated"])
  easeCalibrated <- EmpiricalCalibration::computeExpectedAbsoluteSystematicError(calibratedNull)
  easeCalibratedLabel <- sprintf("EASE: %.2f", easeCalibrated)
  dd$easeLabel <- c(easeCalibratedLabel, easeUncalibratedLabel)
...
prepareTaBalanceTable <- function(taBalance, # taBalance <- targetAnalyticBalance
                                  percentDigits = 1,
                                  stdDiffDigits = 2,
                                  output = "latex") {
  if (output == "latex") {
  ...
  resultsTable <- taBalance[, c("domainId",
                                "covariateName",
                                "meanTarget",
                                "meanAnalytic",
                                "stdDiff")]
  ...
  resultsTable <- resultsTable[order(abs(resultsTable$stdDiff), decreasing = TRUE), ]
  resultsTable$meanTarget <- formatPercent(resultsTable$meanTarget)
  resultsTable$meanAnalytic <- formatPercent(resultsTable$meanAnalytic)
