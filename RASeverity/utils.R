summaryPlpAnalyses <- function(analysesLocation){ 
  ...
  if(!is.null(allPerformance$AUPRC)){
    allPerformance$AUPRC <- as.double(allPerformance$AUPRC)
  } else{
    allPerformance$AUPRC <- -1
  }
  ...
  allPerformance$populationSize[is.na(allPerformance$populationSize)] <- 0
  allPerformance <- allPerformance[allPerformance$populationSize>0,]
...
getPerformance <- function(analysisLocation){
  ...
  return(res[,c('analysisId', 'AUC', 'AUPRC', 'outcomeCount','populationSize','incidence','plpResultLocation', 'plpResultLoad')[c('analysisId', 'AUC', 'AUPRC', 'outcomeCount','populationSize','incidence','plpResultLocation', 'plpResultLoad')%in%colnames(res)] ])
}
...
getValidationPerformance <- function(validationLocation){
  ...
  valPerformance <- valPerformance[,c('analysisId','valDatabase', 'AUC', 'AUPRC', 'outcomeCount','populationSize','incidence')[c('analysisId','valDatabase', 'AUC', 'AUPRC', 'outcomeCount','populationSize','incidence')%in%colnames(valPerformance)]]
