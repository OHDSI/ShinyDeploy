...
getSummary  <- function(result,inputType,validation){
  ...
  sumTab <- sumTab[,c('valDatabase','cohortName','outcomeName','modelSettingName','covariateSettingId','TAR', 'AUC','AUPRC', 'populationSize','outcomeCount','incidence',
                      'plpResultLocation', 'plpResultLoad')]
  colnames(sumTab) <- c( 'Val', 'T', 'O','Model','covariateSettingId', 'TAR', 'AUC','AUPRC', 'T Size','O Count','O Incidence (%)', 'plpResultLocation', 'plpResultLoad')
