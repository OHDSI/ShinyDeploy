...
prepareTable1 <- function(balance,
  ...
  formatPercent <- function(x, idx) {
    value <- round(100 * x, percentDigits)
    value[idx] <- value[idx]/100
    result <- format(value, digits = percentDigits + 1, justify = "right")
    result <- gsub("^-", "<", result)
  ...
  idx <- resultsTable$Characteristic == "    age in years" | 
    resultsTable$Characteristic == "    Inpatient Visit" |
    resultsTable$Characteristic == "    Outpatient Visit" |
    resultsTable$Characteristic == "    Emergency Room Visit" |
    resultsTable$Characteristic == "Distinct medication ingredients"
  
  resultsTable$beforeMatchingMeanTreated <- formatPercent(resultsTable$beforeMatchingMeanTreated, idx)
  resultsTable$beforeMatchingMeanComparator <- formatPercent(resultsTable$beforeMatchingMeanComparator, idx)
  resultsTable$beforeMatchingStdDiff <- formatStdDiff(resultsTable$beforeMatchingStdDiff)
  resultsTable$afterMatchingMeanTreated <- formatPercent(resultsTable$afterMatchingMeanTreated, idx)
  resultsTable$afterMatchingMeanComparator <- formatPercent(resultsTable$afterMatchingMeanComparator, idx)
...
plotPs2 <- function(ps, targetName, comparatorName) {
  ps <- rbind(data.frame(x = ps$preferenceScore, y = ps$targetDensity, group = targetName),
              data.frame(x = ps$preferenceScore, y = ps$comparatorDensity, group = comparatorName))
  ps$group <- factor(ps$group, levels = c(as.character(targetName), as.character(comparatorName)))
  theme1 <- ggplot2::element_text(colour = "#000000", size = 30, margin = ggplot2::margin(0, 0.5, 0, 0.1, "cm"))
  theme2 <- ggplot2::element_text(colour = "#000000", size = 20, margin = ggplot2::margin(0, 0.5, 0, 0.1, "cm"))
  plot <- ggplot2::ggplot(ps,
                          ggplot2::aes(x = x, y = y, color = group, group = group, fill = group)) +
    ggplot2::geom_density(stat = "identity") +
    ggplot2::scale_fill_manual(values = c(rgb(0.8, 0, 0, alpha = 0.5),
...
plotCovariateBalanceScatterPlot <- function(balance,
  ...
  limits <- c(min(c(balance$absBeforeMatchingStdDiff, balance$absAfterMatchingStdDiff),
              ...
              max(c(balance$absBeforeMatchingStdDiff, balance$absAfterMatchingStdDiff),
                  na.rm = TRUE))
  theme <- ggplot2::element_text(colour = "#000000", size = textSize)
  plot <- ggplot2::ggplot(balance, ggplot2::aes(x = absBeforeMatchingStdDiff, y = absAfterMatchingStdDiff)) +
    ggplot2::geom_point(color = rgb(0, 0, 0.8, alpha = 0.3), shape = 16, size = pointSize) +
...
plotCovariateBalanceScatterPlot2 <- function(balance,
                                             beforeLabel = "Before stratification",
                                             afterLabel = "After stratification",
                                             pathToCsv = "selectCovariateIds.csv",
  ...
  limits <- c(min(c(balance$absBeforeMatchingStdDiff, balance$absAfterMatchingStdDiff),
              ...
              max(c(balance$absBeforeMatchingStdDiff, balance$absAfterMatchingStdDiff),
                  na.rm = TRUE))
  theme <- ggplot2::element_text(colour = "#000000", size = textSize)
  selectCovariateIds <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  balance$selectCovariates <- factor(ifelse(balance$covariateId %in% selectCovariateIds$covariateId, "PS model", "Not included"))
  balance$selectCovariates <- factor(balance$selectCovariates, levels = rev(levels(balance$selectCovariates)))
  plot <- ggplot2::ggplot(balance, ggplot2::aes(x = absBeforeMatchingStdDiff, y = absAfterMatchingStdDiff, color = selectCovariates)) +
    ggplot2::scale_color_manual(values=c(rgb(0, 0, 0.8, alpha = 1), rgb(0.8, 0, 0, alpha = 0.2))) +
    ggplot2::geom_point(shape = 16, size = pointSize) +
    ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    ggplot2::geom_hline(yintercept = 0) +
    ggplot2::geom_vline(xintercept = 0) +
    ggplot2::scale_x_continuous(beforeLabel, limits = limits) +
    ggplot2::scale_y_continuous(afterLabel, limits = limits) +
    ggplot2::theme(text = theme, legend.title = ggplot2::element_blank(), legend.position = legendPosition)
  return(plot)
}
...
plotKaplanMeier2 <- function(kaplanMeier, targetName, comparatorName) {
  data <- rbind(data.frame(time = kaplanMeier$time,
                           s = kaplanMeier$targetSurvival,
                           lower = kaplanMeier$targetSurvivalLb,
                           upper = kaplanMeier$targetSurvivalUb,
  ...
  plot <- ggplot2::ggplot(data, ggplot2::aes(x = time,
    ...
    ggplot2::theme(legend.title = ggplot2::element_blank(),
                   ...
                   legend.key.size = ggplot2::unit(1, "lines"),
                   plot.title = ggplot2::element_text(hjust = 0.5),
                   text = ggplot2::element_text(colour = "#000000", size = 30)) +
    ggplot2::theme(axis.title.y = ggplot2::element_text(vjust = -10))
  
  targetAtRisk <- kaplanMeier$targetAtRisk[!is.na(kaplanMeier$targetAtRisk)]
  comparatorAtRisk <- kaplanMeier$comparatorAtRisk[!is.na(kaplanMeier$comparatorAtRisk)]
  labels <- data.frame(x = c(0, xBreaks, xBreaks),
                       y = as.factor(c("At risk (N)",
                                       rep(targetName, length(xBreaks)),
                                       rep(comparatorName, length(xBreaks)))),
                       label = c("",
                                 formatC(targetAtRisk, big.mark = ",", mode = "integer"),
                                 formatC(comparatorAtRisk, big.mark = ",", mode = "integer")))
  labels$y <- factor(labels$y, levels = c(comparatorName, targetName, "At risk (N)"))
  dataTable <- ggplot2::ggplot(labels, ggplot2::aes(x = x, y = y, label = label)) + 
    ggplot2::geom_text(size = 7, vjust = 0.5) + 
    ggplot2::scale_x_continuous(xLabel, limits = xlims, breaks = xBreaks) + 
    ggplot2::theme(panel.grid.major = ggplot2::element_blank(), 
                   panel.grid.minor = ggplot2::element_blank(),
                   legend.position = "none",
