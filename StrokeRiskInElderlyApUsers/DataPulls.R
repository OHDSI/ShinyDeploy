...
getMainResults <- function(connection,
  ...
  result$analysisOrder <- match(result$analysisId, c(1, 3, 5, 2, 4, 6))
  result <- result[order(result$analysisOrder), ]
  result$analysisOrder <- NULL
  return(result)
}
...
getCovariateBalance <- function(connection,
  ...
  balance$databaseId <- factor(balance$databaseId, levels(databaseId))
  balance <- balance[balance$analysisId == analysisId & balance$outcomeId == outcomeId & balance$databaseId == databaseId, ]
  balance <- merge(balance, covariate[covariate$databaseId == databaseId & covariate$analysisId == analysisId, c("covariateId", "covariateAnalysisId", "covariateName")])
...
getPs <- function(connection, targetIds, comparatorIds, databaseId, analysisId) {
  ...
  ps$databaseId <- factor(ps$databaseId, levels(databaseId))
  ps <- ps[ps$databaseId == databaseId & ps$analysisId == analysisId, ]
  return(ps)
}

getPs2 <- function(connection, targetIds, comparatorIds, databaseId, analysisId, dataFolder) {
  file <- sprintf("preference_score_dist_t%s_c%s.rds", targetIds, comparatorIds)
  ps <- readRDS(file.path(dataFolder, file))
  colnames(ps) <- SqlRender::snakeCaseToCamelCase(colnames(ps))
  ps$databaseId <- factor(ps$databaseId, levels(databaseId))
...
getKaplanMeier <- function(connection, targetId, comparatorId, outcomeId, databaseId, analysisId) {
  ...
  km$databaseId <- factor(km$databaseId, levels(databaseId))
...
getKaplanMeier2 <- function(connection, targetId, comparatorId, outcomeId, databaseId, analysisId, dataFolder) {
  file <- sprintf("kaplan_meier_dist_t%s_c%s.rds", targetId, comparatorId)
  km <- readRDS(file.path(dataFolder, file))
  colnames(km) <- SqlRender::snakeCaseToCamelCase(colnames(km))
  km$databaseId <- factor(km$databaseId, levels(databaseId))
